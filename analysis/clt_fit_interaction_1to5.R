# =============================================================================
# clt_fit_interaction_1to5.R
# CLT Fit-Account Interaction Test — Robustness check on 1:5 matched sample
#
# Identical to clt_fit_interaction.R except uses stage9_matched_1to5.parquet.
# Run after clt_fit_interaction.R and compare results for consistency.
#
...
# =============================================================================
# Load and prep matched sample (1:5)
# =============================================================================
cat("\n", strrep("=", 70), "\n", sep = "")
cat("LOADING DATA (1:5 matched sample)\n")
cat(strrep("=", 70), "\n", sep = "")

cat("Loading from parquet (1:5 sample has no prepped RDS) ...\n")
df3 <- arrow::read_parquet(MATCHED_PATH)
...
cat("CLT FIT-ACCOUNT INTERACTION ANALYSIS — 1:5 ROBUSTNESS CHECK\n")
cat(strrep("=", 85), "\n")
cat(sprintf("Date        : %s\n", Sys.time()))
...
cat(format_model_table(mF1, vcov_F1, "MODEL F1 (1:5)"))
...
cat(format_model_table(mF2, vcov_F2, "MODEL F2 (1:5)"))

cat("\n\nINTERPRETATION (F2):\n")
ctF2       <- lmtest::coeftest(mF2, vcov = vcov_F2)
int_soc_nm <- grep("A_social.*A_Soc_OP|A_Soc_OP.*A_social",
                   rownames(ctF2), value = TRUE)[1]
int_hyp_nm <- grep("A_hypothetical.*A_H_OP|A_H_OP.*A_hypothetical",
                   rownames(ctF2), value = TRUE)[1]
...
cat(format_model_table(mF3, vcov_F3, "MODEL F3 (1:5)"))
...
cat("\n=== clt_fit_interaction_1to5.R complete ===\n")
