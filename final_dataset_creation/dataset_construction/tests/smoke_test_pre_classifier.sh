...
# Expect 16 rows: all comments from 4 econ threads (bots included)
# t3_econA1: c1,c1a,c1b,c2 (4) | t3_econB2: c3,c3a,c3b,c4,c4a,c4b (6)
# t3_econC3: c5,c5a,c5b (3)    | t3_econD4: c6,c6a,c6b (3)
_check_jsonl_count "Stage1:row_count" "$SMOKE_OUT/comments_raw.jsonl" 16
...
# Verify delta count: c1(econA1), c3(econB2), c4(econB2), c5(econC3) → 4
...
")
_check_value "Stage1:delta_count" "$DELTA_COUNT" "4"

# Verify bot count: c1b,c3b,c4b,c5b,c6b = 5
BOT_COUNT=$("$PYTHON" -c "
import json
with open('$SMOKE_OUT/stage1_counts.json') as f:
    d = json.load(f)
...
")
_check_value "Stage1:bot_count" "$BOT_COUNT" "5"

# Verify is_award_gesture flag on award gestures: c1a,c3a,c4a,c5a = 4 in non-bot rows
...
")
_check_value "Stage1:award_gesture_count" "$AG_COUNT" "4"
...
# Expected survivors: c1, c2, c3, c4, c6 = 5
# Removed: bots(5) + self-delta c5(1) + award gestures c1a,c3a,c4a,c5a(4) + short c6a(1) = 11
_check_jsonl_count "Stage2:row_count" "$SMOKE_OUT/comments_filtered.jsonl" 5
...
_check_jsonl_count "Stage3:row_count" "$SMOKE_OUT/comments_segmented.jsonl" 5
...
print(ok)
")
_check_value "Stage3:all_have_sentences" "$HAS_SENTS" "5"
